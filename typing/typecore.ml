      ...
      Longident.Lident s -> begin
        ...
        with Not_found ->
          raise (Error (lid.loc, env,
                        Wrong_name ("", newvar Stype, type_kind, tpath, s, names)))
      end
    | _ -> raise Not_found
...
let rec type_approx env sexp =
  ...
  | Pexp_function ({pc_rhs=e}::_) ->
      ...
      let ty2 = newvar Seffect in
      let ty3 = type_approx env e in
       newty (Tarrow(Nolabel, ty1, ty2, ty3, Cok))
...
let rec type_exp ?recarg env sexp eff_expected =
  (* We now delegate everything to type_expect *)
  type_expect ?recarg env sexp (newvar Stype) eff_expected
...
and type_expect ?in_function ?recarg env sexp ty_expected eff_expected =
  let previous_saved_types = Cmt_format.get_saved_types () in
  Typetexp.warning_enter_scope ();
  Typetexp.warning_attribute sexp.pexp_attributes;
  let exp =
    type_expect_ ?in_function ?recarg env sexp ty_expected eff_expected
  in
...
and type_expect_ ?in_function ?(recarg=Rejected) env sexp
  ...
  | Pexp_match(sarg, caselist) ->
      ...
      let val_cases, partial =
        type_cases env arg.exp_type eff_expected ty_expected
          true loc val_caselist
      in
      let exn_cases, _ =
        type_cases env Predef.type_exn eff_expected ty_expected
  ...
  | Pexp_try(sbody, caselist) ->
      let body = type_expect env sbody ty_expected eff_expected in
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let opt_exp =
        ...
        | Some sexp ->
            if !Clflags.principal then begin_def ();
            let exp = type_exp ~recarg env sexp eff_expected in
            if !Clflags.principal then begin
              end_def ();
...
and type_function ?in_function loc attrs env ty_expected l caselist =
  ...
  let cases, partial =
    type_cases ~in_function:(loc_fun,ty_fun) env ty_arg ty_eff ty_res
...
and type_label_access env loc srecord lid eff_expected =
  if !Clflags.principal then begin_def ();
  let record = type_exp ~recarg:Allowed env srecord eff_expected in
  if !Clflags.principal then begin
    end_def ();
...
and type_argument ?recarg env sarg ty_expected' ty_expected eff_expected =
  ...
  in
  match expand_head env ty_expected' with
    {desc = Tarrow(Nolabel,ty_arg,_,ty_res,_); level = lv} when is_inferred sarg ->
      ...
      let rec make_args args ty_fun =
        ...
        | Tarrow (l,_,_,ty_res',_) when l = Nolabel || !Clflags.classic ->
      ...
      Location.prerr_warning texp.exp_loc
        (Warnings.Eliminated_optional_arguments optional_arg_names);
  ...
  | _ ->
      let texp = type_expect ?recarg env sarg ty_expected' eff_expected in
      unify_exp env texp ty_expected;
      texp
...
and type_construct env loc lid sarg ty_expected eff_expected attrs =
  ...
  in
  let args =
    List.map2 (fun e (t,t0) -> type_argument ~recarg env e t t0 eff_expected)
...
and type_cases ?in_function env ty_arg ty_eff ty_res
...
and type_effect_cases env ty_res eff_expected loc caselist conts =
  let ty = newvar Stype in
  (* remember original level *)
  begin_def ();
  ...
  let (id, new_env) = Env.enter_type name decl env in
  Ctype.init_def(Ident.current_time());
  let ty_eff = newgenty (Tconstr (Path.Pident id,[],Stype,ref Mnil)) in
  ...
  let cases, _ =
    type_cases new_env ty_arg eff_expected ty_res ~conts false loc caselist
  in
  end_def ();
  cases
  ...
  let exp_list =
    List.map2
      (fun {pvb_expr=sexp; _} (pat, slot) ->
        ...
        | Tpoly (ty, tl) ->
            ...
            let exp =
              Typetexp.with_warning_attribute pvb_attributes (fun () ->
                type_expect exp_env sexp ty' eff_expected)
            in
            end_def ();
        ...
        | _ ->
            Typetexp.with_warning_attribute pvb_attributes (fun () ->
              type_expect exp_env sexp pat.pat_type eff_expected))
