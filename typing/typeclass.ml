...
and class_type env scty =
  ...
  | Pcty_arrow (l, sty, scty) ->
      ...
      let ty =
        ...
        then Ctype.newty (Tconstr(Predef.path_option,[ty], Stype, ref Mnil))
        else ty in
  ...
  | Pcf_method (lab, priv, Cfk_concrete (ovf, expr))  ->
      ...
      let field =
        lazy begin
          ...
          in
          let meth_type =
            Btype.newgenty (Tarrow(Nolabel, self_type, eff, ty, Cok)) in
...
and class_structure cl_num final val_env met_env loc
  ...
  (* if !Clflags.principal then *) begin
    ...
    Meths.iter (fun _ (_,ty) -> Ctype.unify val_env ty (Ctype.newvar Stype)) ms
  end;
